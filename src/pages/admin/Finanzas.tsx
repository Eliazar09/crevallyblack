  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-medium text-cream-50">Finanzas</h1>
          <p className="text-sm text-ink-500">Resumen del mes actual</p>
        </div>
      ...
      {chart.length > 0 && (
        <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:0.2}}
          className="bg-forest-900/60 border border-white/5 rounded-2xl p-5">
          <p className="text-sm font-semibold text-cream-100 mb-4">Evolución mensual</p>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <p className="text-sm font-semibold text-cream-100">Transacciones</p>
          <div className="flex gap-1.5">
            {['all','ingreso','egreso'].map((t) => (
              <button key={t} onClick={() => setFilterType(t)}
                className={cn('px-3 py-1 rounded-full text-xs font-medium transition-colors',
                  filterType === t ? 'bg-gold-400 text-forest-950' : 'bg-white/5 text-ink-500 hover:text-cream-200')}>
                  ...
                  <motion.tr key={t.id} initial={{opacity:0,y:4}} animate={{opacity:1,y:0}} transition={{delay:i*0.02}}
                    ...
                    <td className="px-4 py-3">
                      <span className={cn('px-2 py-0.5 rounded-full text-[11px] font-medium',
                        t.type === 'ingreso' ? 'text-green-400 bg-green-400/10' : 'text-red-400 bg-red-400/10')}>
                        {t.type === 'ingreso' ? 'Ingreso' : 'Egreso'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-ink-500 text-xs">{t.category}</td>
                    <td className="px-4 py-3 text-cream-200 max-w-[200px] truncate">{t.description ?? '—'}</td>
                    <td className={cn('px-4 py-3 font-mono font-medium', t.type === 'ingreso' ? 'text-green-400' : 'text-red-400')}>
                      {t.type === 'ingreso' ? '+' : '-'}{formatPrice(t.amount)}
                    </td>
                    <td className="px-4 py-3 text-ink-500 text-xs">{new Date(t.created_at).toLocaleDateString('es-VE')}</td>
                  </motion.tr>
                ))}
              </tbody>
            </table>
      ...
      <Modal isOpen={modalOpen} onClose={()=>setModalOpen(false)} title="Nueva transacción">
        <div className="p-5 space-y-4">
            ...
            {(['ingreso','egreso'] as const).map((t)=>(
              <button key={t} onClick={()=>{setTxType(t);setTxCat(t==='ingreso'?'Otro ingreso':'Otro egreso')}}
                className={cn('py-2.5 rounded-xl text-sm font-semibold transition-colors',
                  ...
                  : 'bg-white/5 text-ink-500 border border-white/10 hover:text-cream-200')}>
          ...
          <div className="flex gap-3 justify-end pt-2">
            ...
            <button onClick={handleCreate} disabled={saving} className="px-5 py-2 rounded-full text-sm font-semibold bg-gold-400 text-forest-950 hover:bg-gold-300 disabled:opacity-60">
