...
export default function Estoque() {
  ...
  async function handleMovement() {
    if (!selProduct) { push('Selecione um produto', 'error'); return }
    if (movQty < 1) { push('A quantidade deve ser maior que 0', 'error'); return }
    setSaving(true)
    try {
      await createMovement({ product_id: selProduct, type: movType as any, quantity: movQty, reason: movReason || movType })
      push('Movimentação registrada')
      setModalOpen(false); setMovQty(1); setMovReason(''); setSelProduct('')
      load()
    } catch { push('Erro ao registrar movimentação', 'error') }
    finally { setSaving(false) }
  }

  const typeLabel: Record<string, string> = { entrada:'Entrada', saida:'Saída', ajuste:'Ajuste' }
  const typeColor: Record<string, string> = { entrada:'text-emerald-600 bg-emerald-50', saida:'text-red-600 bg-red-50', ajuste:'text-blue-600 bg-blue-50' }

  return (
    <div className="p-4 sm:p-6 space-y-5 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl text-gray-900 tracking-wide">ESTOQUE</h1>
      ...
      {lowStock.length > 0 && (
        <motion.div initial={{opacity:0}} animate={{opacity:1}}
          ...
          <div>
            <p className="text-sm font-semibold text-amber-800">Alerta de estoque baixo ({lowStock.length} produtos)</p>
      ...
      {loading ? (
        ...
        <EmptyState icon={Boxes} title="Sem produtos" description="Crie produtos para gerenciar o estoque." />
      ) : (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                {['Produto','Categoria','Estoque','Mín.','Status'].map((h)=>(
      ...
      <div>
        <p className="text-sm font-semibold text-gray-900 mb-3">Últimas movimentações</p>
        {movements.length === 0 ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  {['Produto','Tipo','Qtd','Motivo','Data'].map((h)=>(
                ...
                {movements.slice(0,20).map((m) => (
                  <tr key={m.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-2.5">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${typeColor[m.type] ?? 'text-gray-600 bg-gray-100'}`}>
                    ...
                    <td className="px-4 py-2.5 text-gray-400 text-xs">{new Date(m.created_at).toLocaleDateString('pt-BR')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Modal isOpen={modalOpen} onClose={()=>setModalOpen(false)} title="Ajustar estoque">
        <div className="p-5 space-y-4">
          <p className="font-semibold text-gray-900 pr-8">Ajustar estoque</p>
          ...
          <div className="space-y-1.5">
            ...
            <input value={movReason} onChange={(e)=>setMovReason(e.target.value)} className={field} placeholder="Ex: Compra fornecedor, Avaria…" />
          </div>
          <div className="flex gap-3 justify-end pt-2">
            ...
            <button onClick={handleMovement} disabled={saving} className="px-5 py-2 rounded-full text-sm font-semibold bg-ink-900 text-white hover:bg-ink-700 disabled:opacity-60">
