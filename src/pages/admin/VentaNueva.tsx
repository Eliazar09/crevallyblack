...
import { createSale, methodLabels, type PaymentMethod, type PaymentStatus, type SaleItem } from '../../lib/queries/sales'
...
export default function VendaNova() {
  ...
  useEffect(() => {
    getProducts({ status: 'ativo' }).then(setProducts)
  ...
  async function createQuickClient() {
    ...
    try {
      ...
      push('Cliente criado')
      return c
    } catch { push('Erro ao criar cliente', 'error'); return null }
  }

  async function handleSubmit() {
    if (cart.length === 0) { push('Adicione ao menos um produto', 'error'); return }
    const clientName = selectedClient?.name ?? guestName.trim()
    if (!clientName) { push('Informe o nome do cliente', 'error'); return }
    setSaving(true)
    try {
      ...
      push('Venda registrada com sucesso!')
      ...
      if (selectedClient?.phone) {
        const link = buildWhatsAppLink(
          ...
          { nome: clientName, telefone: selectedClient.phone, cidade: (selectedClient as any).city ?? '' }
        )
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-display text-2xl text-gray-900 tracking-wide">NOVA VENDA</h1>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Esquerda: produtos + cliente */}
          ...
          <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-5 space-y-3">
            ...
            {selectedClient ? (
              <div className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3">
                ...
                <button onClick={() => setSelectedClient(null)} className="text-gray-500 hover:text-red-400 text-xs">Trocar</button>
              </div>
            ) : (
              <>
                ...
                {!selectedClient && (
                  <div className="space-y-2">
                    <p className="text-xs text-gray-500">Ou informe o nome diretamente:</p>
                    <input value={guestName} onChange={(e) => setGuestName(e.target.value)}
                      placeholder="Nome do cliente" className={field} />
                  </div>
                )}
                <button onClick={() => setShowNewClient(!showNewClient)}
                  className="text-xs text-coffee-600 hover:text-coffee-500 flex items-center gap-1">
                  <Plus size={12} />Criar novo cliente
                </button>
                {showNewClient && (
                  <div className="space-y-2 border border-gray-200 rounded-xl p-3">
                    <input value={newClientName} onChange={(e) => setNewClientName(e.target.value)} className={field} placeholder="Nome *" />
                    <input value={newClientPhone} onChange={(e) => setNewClientPhone(e.target.value)} className={field} placeholder="Telefone" />
                    <button onClick={createQuickClient} className="px-4 py-2 rounded-xl bg-coffee-50 text-coffee-700 text-xs font-semibold hover:bg-coffee-100">Salvar cliente</button>
                  </div>
                )}
              </>
            )}
          ...
          <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-5 space-y-3">
            ...
            <div className="relative">
              ...
              <input value={prodSearch} onChange={(e) => setProdSearch(e.target.value)}
                placeholder="Buscar produto por nome ou SKU…" className={field + ' pl-9'} />
            </div>
            {prodSearch && (
              <div className="border border-gray-200 rounded-xl overflow-hidden">
                {filteredProds.map((p) => (
                  <button key={p.id} onClick={() => addToCart(p)}
                    ...
                    <span>{p.name}</span>
                    <span className="font-mono text-coffee-600">{formatPrice(p.price)}</span>
                  </button>
                ))}
                {filteredProds.length === 0 && <p className="px-4 py-3 text-sm text-gray-500">Nenhum resultado</p>}
              </div>
            )}
                ...
                {cart.map((e) => (
                  <div key={e.product.id} className="flex items-center gap-3 bg-gray-50 rounded-xl px-3 py-2.5">
                    ...
                    <input type="number" value={e.unitPrice} onChange={(ev) => setPrice(e.product.id, Number(ev.target.value))}
                      className="w-24 bg-gray-50 border border-gray-200 rounded-lg px-2 py-1 text-sm font-mono text-gray-900 text-center focus:outline-none focus:border-coffee-400/40" />
                    <span className="font-mono text-xs text-coffee-600 w-20 text-right">{formatPrice(e.unitPrice * e.quantity)}</span>
                    <button onClick={() => removeFromCart(e.product.id)} className="text-gray-500 hover:text-red-400"><Trash2 size={13}/></button>
                  </div>
                ))}
              </div>
            )}
            {cart.length === 0 && <p className="text-xs text-gray-500 text-center py-4">Busque e adicione produtos ao pedido</p>}
          </div>
        </div>
          ...
          <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-5 space-y-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between items-center">
                ...
                <input type="number" min="0" value={discount} onChange={(e) => setDiscount(Number(e.target.value))}
                  className="w-24 bg-gray-50 border border-gray-200 rounded-lg px-2 py-1 text-sm font-mono text-gray-900 text-right focus:outline-none focus:border-coffee-400/40" />
              </div>
              <div className="flex justify-between font-semibold text-gray-900 border-t border-gray-200 pt-2">
                <span>Total</span><span className="font-mono text-coffee-600">{formatPrice(total)}</span>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2}
                className={field + ' resize-none'} placeholder="Observações…" />
            </div>

            <button onClick={handleSubmit} disabled={saving || cart.length === 0}
              className="w-full py-3 rounded-xl bg-ink-900 text-white font-semibold text-sm hover:bg-ink-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
              {saving ? 'Registrando…' : 'Registrar venda'}
            </button>

            {selectedClient?.phone && (
              <p className="text-xs text-gray-500 text-center flex items-center justify-center gap-1">
                <MessageCircle size={11} />Confirmação será enviada por WhatsApp
              </p>
            )}
          </div>
        </div>
